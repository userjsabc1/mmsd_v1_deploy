...
def _resolve_lm_head(base_model):
    if hasattr(base_model, "lm_head"):
        return base_model.lm_head
    if hasattr(base_model, "language_model") and hasattr(
    ...
    if hasattr(base_model, "model") and hasattr(base_model.model, "lm_head"):
        return base_model.model.lm_head
    raise AttributeError("Cannot resolve lm_head from base model")


def _spec_device(model):
    if hasattr(model.spec_layer, "embed_tokens"):
        return model.spec_layer.embed_tokens.weight.device
    if hasattr(model.spec_layer, "fc"):
        return model.spec_layer.fc.weight.device
    return None
...
def initialize_tree(
    ...
    **kwargs,
    ...
    # For VLM: replace image tokens with pad in the input_ids copy given to
    # topK_genrate so the spec model uses pad embeddings at image positions
    # whose hidden_states have already been zeroed out.
    ...
    lm_head = _resolve_lm_head(model.base_model)
    spec_device = _spec_device(model)
    if spec_device is not None and hidden_states.device != spec_device:
        hidden_states = hidden_states.to(spec_device)
    if spec_device is not None and inputs_embeds is not None:
        inputs_embeds = inputs_embeds.to(spec_device)
...
def tree_decoding(
    ...
    if model.base_model.config.architectures[0] == "Qwen2_5_VLForConditionalGeneration":
        rope_deltas = model.base_model.rope_deltas.to(position_ids.device)
        position_ids = (
            position_ids.unsqueeze(0) + rope_deltas
        )  # Qwen2_5_VL
...
def update_inference_inputs(
    ...
    lm_head = _resolve_lm_head(model.base_model)
    spec_device = _spec_device(model)
    if spec_device is not None and accept_hidden_state_new.device != spec_device:
        accept_hidden_state_new = accept_hidden_state_new.to(spec_device)
    draft_tokens, retrieve_indices, tree_mask, tree_position_ids = (
        model.spec_layer.topK_genrate(
